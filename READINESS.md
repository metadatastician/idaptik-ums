...
| SPARK/GNATprove reference model (`spark/`) | D | Alpha-unstable | `ZonesOrdered` stated as SPARK contracts; the linear sweep's equivalence to the quadratic specification is the postcondition. Compiles under GNAT 14.2, and `just spark-parity` drives 21 shared vectors through the SPARK model and the Rust engine with identical verdicts (negative-tested three ways). **D, not C: the verification conditions have NOT been discharged locally — `gnatprove` is not installed on the development machine, and `spark-ci.yml`'s proof job has never run.** | 2026-07-22 |
| Zig hexadeca connector | D | Alpha-unstable | Wire contract built and gated: 16 connectors generated from `config/connectors.ncl` into Zig, Rust and Idris2, with 7 tests proving cross-language ordinal agreement (negative-tested by swapping two Zig ordinals). **0 of 16 transports implemented**; `dispatch` returns `NotImplemented` for every tag rather than a success code. D, not C, because the layer carries no traffic. | 2026-07-22 |
